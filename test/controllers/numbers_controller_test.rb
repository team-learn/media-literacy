...
class NumbersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create number" do
    assert_difference('Number.count') do
      post numbers_url, params: { number: { sentence: @number.sentence, sentence_id: @number.sentence_id } }
    end
  ...
  test "should update number" do
    patch number_url(@number), params: { number: { sentence: @number.sentence, sentence_id: @number.sentence_id } }
